...
export const Sidebar: React.FC<Props> = ({ navigations }) => {
  ...
  return (
    <div className="relative">
      ...
      >
        <div className="flex items-center justify-between flex-shrink-0 py-2 px-6 sm:pr-0">
          <Link to="/" className="flex items-center">
            <img src="/logo.png" alt="logo di" width={20} height={20} />
            <h1 className="text-xl leading-none ml-2 text-primary-400 hidden 2xl:block sidebar-expanded:block">
              ARservices
            </h1>
          </Link>
        ...
        <div className="space-y-8 mt-6">
          {navigations.map(({ title, routes }, i) => (
            <div key={`section_${i}`}>
              {title && (
                <h3 className="text-xs text-gray-500 flex items-center leading-none mb-4">
                  {/* <span
